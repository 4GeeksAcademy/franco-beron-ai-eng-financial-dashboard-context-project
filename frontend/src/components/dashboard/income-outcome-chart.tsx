...
export function IncomeOutcomeChart({ data, loading }: IncomeOutcomeChartProps) {
  ...
  const screenReaderSummary = data
    .map(
      (point) =>
        `${point.month}: income ${formatCurrency(point.income)}, outcome ${formatCurrency(point.outcome)}`,
    )
    .join(". ");
    ...
    <Card className="border-border/60">
      ...
      <CardContent>
        {!hasData ? (
          ...
          <figure aria-label="Line chart of monthly income and outcome">
            ...
            <figcaption className="sr-only">
              Monthly income and outcome summary. {screenReaderSummary}
            </figcaption>
          </figure>
        )}
      </CardContent>
