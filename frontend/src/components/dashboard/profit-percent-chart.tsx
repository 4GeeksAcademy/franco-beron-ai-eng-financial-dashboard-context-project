...
export function ProfitPercentChart({ data, loading }: ProfitPercentChartProps) {
  ...
  const screenReaderSummary = data
    .map((point) => `${point.month}: ${point.profitPercent.toFixed(1)} percent`)
    .join(". ");
    ...
    <Card className="border-border/60">
      ...
      <CardContent>
        {!hasData ? (
          ...
          <figure aria-label="Line chart of monthly profit margin percentage">
            ...
            <figcaption className="sr-only">
              Monthly profit margin summary. {screenReaderSummary}
            </figcaption>
          </figure>
        )}
      </CardContent>
